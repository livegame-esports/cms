...
class GetData {
	...
	public function stats($start, $server, $limit = 30, $name = null, $by_steam_id = null) {
		...
		if($type == 2) {
			$STH = $pdo2->query("SELECT * FROM csstats_settings");
			$STH->setFetchMode(PDO::FETCH_OBJ);
			...
			while($row = $STH->fetch()) {
				if($row->command == 'ar_levels') {
					...
					$skills = explode(" ", $row->value);
				}elseif($row->command == 'statsx_skillname') {
			...
			if($army_enable > 0) {
				...
				$STH = $pdo2->prepare("SELECT * FROM csstats_settings WHERE command LIKE :name"); $STH->setFetchMode(PDO::FETCH_OBJ);
		...
		if(empty($name)) {
			if($type == 1 || $type == 2) {
				if(check_table('csstats_extra_stats', $pdo2)) {
					$STH = $pdo2->query("SELECT * FROM csstats_players LEFT JOIN `csstats_extra_stats` USING (`authid`) ORDER BY $sort LIMIT $start, $limit");
				} else {
					$STH = $pdo2->query("SELECT * FROM csstats_players ORDER BY $sort LIMIT $start, $limit");
				}
			...
			if($type == 1 || $type == 2) {
				if(check_table('csstats_extra_stats', $pdo2)) {
					$STH = $pdo2->prepare("SELECT * FROM csstats_players LEFT JOIN `csstats_extra_stats` USING (`authid`) WHERE (nick LIKE :name or authid LIKE :name or ip LIKE :name) ORDER BY $sort");
				} else {
		...
		while($row = $STH->fetch()) {
			...
			if($army_enable === 0 || $army_enable == 2) {
			...
			if($type == 2) {
				...
				$this->tpl->set("{rank}", $army_enable == 1 ? $rank : $ranks[$row->cms_level]);
				$this->tpl->set("{level}", $army_enable == 1 ? $level : $row->cms_xp + $row->cms_addxp);
				if($army_enable == 1) {
			...
			if($type == 1 || $type == 2 || $type == 3 || $type == 5 || $type == 6) {
				...
				$this->tpl->set("{authid_original}", $row->authid);
			}
			if($type == 1 || $type == 2 || $type == 3 || $type == 5) {
			...
			$this->tpl->set("{kdr}", ($row->deaths == 0) ? $row->frags : round($row->frags/$row->deaths, 2));
	...
	public function notifications($start, $limit = 10, $isNeedNew = false) {
		...
		if($isNeedNew) {
			$STH = $this->pdo->query("SELECT * FROM `notifications` WHERE user_id='$_SESSION[id]' and status='0' ORDER BY date DESC $limitExpression");
		} else {
			$STH = $this->pdo->query("SELECT * FROM notifications WHERE user_id='$_SESSION[id]' ORDER BY date DESC $limitExpression");
		}
