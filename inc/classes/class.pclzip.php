...
class PclZip
{
	...
	public function properties()
	{
		...
		if(@is_file($this->zipname)) {
			...
			$v_central_dir = [];
			if(($v_result = $this->privReadEndCentralDir(
					$v_central_dir
				)) != 1) {
				$this->privSwapBackMagicQuotes();
	...
	public function merge($p_archive_to_add)
	{
		...
		if((is_object($p_archive_to_add)) && (get_class(
					$p_archive_to_add
				) == 'pclzip')) {
	...
	public function privParseOptions(
			...
			switch($p_options_list[$i]) {
				...
				case PCLZIP_OPT_ADD_PATH:
					...
					if(($i + 1) >= $p_size) {
						...
						PclZip::privErrorLog(
							...
							"Missing parameter value for option '" . PclZipUtilOptionText(
								$p_options_list[$i]
							) . "'"
						);
				...
				case PCLZIP_OPT_TEMP_FILE_THRESHOLD:
					...
					if((!is_integer($v_value)) || ($v_value < 0)) {
						PclZip::privErrorLog(
							...
							"Integer expected for option '" . PclZipUtilOptionText(
								$p_options_list[$i]
							) . "'"
						);
				...
				case PCLZIP_OPT_BY_NAME:
					...
					if(is_string($p_options_list[$i + 1])) {
						...
						PclZip::privErrorLog(
							PCLZIP_ERR_INVALID_OPTION_VALUE,
							"Wrong parameter value for option '" . PclZipUtilOptionText(
								$p_options_list[$i]
							) . "'"
						);
				...
				case PCLZIP_OPT_BY_INDEX:
					...
					for($j = 0; $j < sizeof($v_work_list); $j++) {
						...
						if($v_result_list[$p_options_list[$i]][$j]['start'] < $v_sort_value) {
							...
							PclZip::privErrorLog(
								PCLZIP_ERR_INVALID_OPTION_VALUE,
								"Invalid order of index range for option '" . PclZipUtilOptionText(
									$p_options_list[$i]
								) . "'"
							);
				...
				case PCLZIP_CB_POST_ADD:
                    ...
                    */
					...
					if(!function_exists($v_function_name)) {
						...
						PclZip::privErrorLog(
							PCLZIP_ERR_INVALID_OPTION_VALUE,
							"Function '" . $v_function_name . "()' is not an existing function for option '" . PclZipUtilOptionText(
								$p_options_list[$i]
							) . "'"
						);
		...
		if($v_requested_options !== false) {
			for($key = reset($v_requested_options); $key = key(
				$v_requested_options
				...
				if($v_requested_options[$key] == 'mandatory') {
					...
					if(!isset($v_result_list[$key])) {
						...
						PclZip::privErrorLog(
							PCLZIP_ERR_INVALID_PARAMETER,
							"Missing mandatory parameter " . PclZipUtilOptionText(
								$key
							) . "(" . $key . ")"
						);
			...
			switch($v_key) {
				case PCLZIP_ATT_FILE_NAME:
					if(!is_string($v_value)) {
						PclZip::privErrorLog(
							...
							"Invalid type " . gettype(
								$v_value
							) . ". String expected for attribute '" . PclZipUtilOptionText(
								$v_key
							) . "'"
						);
					...
					if($p_filedescr['filename'] == '') {
						PclZip::privErrorLog(
							PCLZIP_ERR_INVALID_ATTRIBUTE_VALUE,
							"Invalid empty filename for attribute '" . PclZipUtilOptionText(
								$v_key
							) . "'"
						);
				...
				case PCLZIP_ATT_FILE_NEW_SHORT_NAME:
					...
					if($p_filedescr['new_short_name'] == '') {
						PclZip::privErrorLog(
							PCLZIP_ERR_INVALID_ATTRIBUTE_VALUE,
							"Invalid empty short filename for attribute '" . PclZipUtilOptionText(
								$v_key
							) . "'"
						);
				...
				case PCLZIP_ATT_FILE_NEW_FULL_NAME:
					...
					if($p_filedescr['new_full_name'] == '') {
						PclZip::privErrorLog(
							PCLZIP_ERR_INVALID_ATTRIBUTE_VALUE,
							"Invalid empty full filename for attribute '" . PclZipUtilOptionText(
								$v_key
							) . "'"
						);
				...
				case PCLZIP_ATT_FILE_MTIME:
					if(!is_integer($v_value)) {
						PclZip::privErrorLog(
							...
							"Invalid type " . gettype(
								$v_value
							) . ". Integer expected for attribute '" . PclZipUtilOptionText(
								$v_key
							) . "'"
						);
	...
	public function privAddFileList(
		...
		for($j = 0; ($j < sizeof(
			...
			// ----- Check the filename
			if(($p_filedescr_list[$j]['type'] != 'virtual_file') && (!file_exists(
					$p_filedescr_list[$j]['filename']
				))) {
				PclZip::privErrorLog(
					PCLZIP_ERR_MISSING_FILE,
		...
		if($p_header['status'] == 'ok') {
			...
			if($p_filedescr['type'] == 'file') {
				...
				if((!isset($p_options[PCLZIP_OPT_TEMP_FILE_OFF])) && (isset($p_options[PCLZIP_OPT_TEMP_FILE_ON]) || (isset($p_options[PCLZIP_OPT_TEMP_FILE_THRESHOLD]) && ($p_options[PCLZIP_OPT_TEMP_FILE_THRESHOLD] <= $p_header['size'])))) {
					...
					if(($v_result = $this->privWriteFileHeader(
							$p_header
						)) != 1) {
						@fclose($v_file);
	...
	public function privCalculateStoredFilename(&$p_filedescr, &$p_options)
	{
		...
		if(isset($p_filedescr['new_full_name'])) {
			...
			if($p_remove_all_dir) {
				...
				if((substr($p_filename, 0, 2) == "./") || (substr(
							$p_remove_dir,
							0,
							2
						) == "./")) {
		...
		$v_stored_filename              = PclZipUtilPathReduction(
			$v_stored_filename
		);
	...
	public function privConvertHeader2FileInfo($p_header, &$p_info)
	{
		...
		$v_temp_path               = PclZipUtilPathReduction(
			$p_header['filename']
		);
	...
	public function privExtractByRule(
		...
		// ----- Check the path
		if(($p_path == "") || ((substr($p_path, 0, 1) != "/") && (substr(
						$p_path,
						0,
						3
					) != "../") && (substr($p_path, 1, 2) != ":/"))) {
		...
		for($i = 0, $v_nb_extracted = 0; $i < $v_central_dir['entries']; $i++) {
			...
			if((isset($p_options[PCLZIP_OPT_BY_NAME])) && ($p_options[PCLZIP_OPT_BY_NAME] != 0)) {
				...
				for($j = 0; ($j < sizeof(
					...
					if(substr($p_options[PCLZIP_OPT_BY_NAME][$j], -1) == "/") {
						...
						if((strlen($v_header['stored_filename']) > strlen(
            ...
            */
				...
				for($j = $j_start; ($j < sizeof(
						$p_options[PCLZIP_OPT_BY_INDEX]
					)) && (!$v_extract); $j++) {
	...
	public function privExtractFile(
		...
		if($p_entry['status'] == 'ok') {
			...
			if(file_exists($p_entry['filename'])) {
				...
				if((($p_entry['external'] & 0x00000010) == 0x00000010) || (substr(
							$p_entry['filename'],
							-1
						) == '/')) {
	...
	public function privReadEndCentralDir(&$p_central_dir)
	{
		...
		if(strlen($v_binary_data) != 18) {
			...
			PclZip::privErrorLog(
				PCLZIP_ERR_BAD_FORMAT,
				"Invalid End of Central Dir Record size : " . strlen(
					$v_binary_data
				)
			);
	...
	public function privDeleteByRule(&$p_result_list, &$p_options)
	{
		...
		if($v_nb_extracted > 0) {
			...
			// ----- Creates a temporay file
			$v_zip_temp_name = PCLZIP_TEMPORARY_DIR . uniqid(
					'pclzip-'
				) . '.tmp';
	...
	public function privDisableMagicQuotes()
	{
		...
		if((!function_exists("get_magic_quotes_runtime")) || (!function_exists(
...
function PclZipUtilPathInclusion($p_dir, $p_path)
{
	...
	if(($p_dir == '.') || ((strlen($p_dir) >= 2) && (substr(
					$p_dir,
					0,
					2
				) == './'))) {
		$p_dir = PclZipUtilTranslateWinPath(getcwd(), false) . '/' . substr(
				$p_dir,
				1
			);
	}
	if(($p_path == '.') || ((strlen($p_path) >= 2) && (substr(
					$p_path,
					0,
					2
				) == './'))) {
		$p_path = PclZipUtilTranslateWinPath(getcwd(), false) . '/' . substr(
				$p_path,
				1
			);
	}
...
function PclZipUtilTranslateWinPath($p_path, $p_remove_disk_letter = true)
{
	if(stristr(php_uname(), 'windows')) {
		...
		if(($p_remove_disk_letter) && (($v_position = strpos(
					$p_path,
					':'
				)) != false)) {
