...
class Forum {
	...
	public function add_forum($section_id, $name, $description, $img) {
		...
		$STH->setFetchMode(PDO::FETCH_OBJ);
		$access = $STH->fetch();
		if(!self::have_rights($access->access)) {
			return false;
		}
	...
	public function get_forums_admin($section_id, $template) {
	...
	public function get_forums() {
		...
		if($count_forums != 0) {
			for($i_forums = 0; $i_forums < $count_forums; $i_forums++) {
				if(self::have_rights($forums[$i_forums]['access'])) {
	...
	public function get_forum($id, $start, $limit = 20) {
		...
		$STH->setFetchMode(PDO::FETCH_OBJ);
		$row = $STH->fetch();
		if(!self::have_rights($row->access)) {
	...
	public static function have_rights($groups) {
		if($groups == ';') {
			return true;
		}
