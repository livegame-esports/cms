		...
		<div class="col-md-6">
			...
			<div class="block">
				...
				<div class="input-group">
					...
					<select id="user_gag" class="form-control">
						<option value="1" <?php if('{gag}' == '1'): ?>selected<?php endif; ?>>Включен</option>
						<option value="2" <?php if('{gag}' == '2'): ?>selected<?php endif; ?>>Выключен</option>
					</select>
				</div>
				<br>
				...
				<div class="bs-callout bs-callout-info mt--10 mb-10">
					<p>
						При включенном муте пользователь не может оставлять записи в публичных местах:
					    сообщения в чате и на форуме, комментарии к новостям и заявкам, создавать темы на форуме
					</p>
				</div>
			...
			<script>
				$(document).ready(function() {
					init_tinymce('signature', '<?php echo md5($conf->code); ?>', 'lite');
				});
		...
		<div class="col-md-6">
			...
			<div class="block">
				...
				<table class="table table-striped table-bordered mb-0">
					...
					<tbody>
						<?php if('{multi_account}' == '0'): ?>
						<tr>
							<td colspan="3">Профили не найдены</td>
						</tr>
						<?php else: ?>
							<?php for($i=0; $i < count($multi_accounts); $i++): ?>
								<?php if($multi_accounts[$i][0] != 0): ?>
								<tr id="multi-account-<?php echo $multi_accounts[$i][0]; ?>">
									<td>
										<a href="../admin/edit_user?id=<?php echo $multi_accounts[$i][0]; ?>" target="_blank"><?php echo $multi_accounts[$i][2]; ?></a>
										<span class="glyphicon glyphicon-remove c-p fs-12" onclick="dell_multi_account_relation({id}, <?php echo $multi_accounts[$i][0]; ?>); dell_block('multi-account-<?php echo $multi_accounts[$i][0]; ?>');" title="Очистить связь с профилем"></span>
										<span class="glyphicon glyphicon-trash c-p fs-12" onclick="dell_user(<?php echo $multi_accounts[$i][0]; ?>, 2, 1); dell_block('multi-account-<?php echo $multi_accounts[$i][0]; ?>');" title="Удалить профиль"></span>
									</td>
									<td>
										<?php if($multi_accounts[$i][1] == 1): ?>
											Совпадение по IP
										<?php endif; ?>
										<?php if($multi_accounts[$i][1] == 2): ?>
											<p class="text-warning mb-0">Совпадение по ОС и информации браузера</p>
										<?php endif; ?>
										<?php if($multi_accounts[$i][1] == 3): ?>
