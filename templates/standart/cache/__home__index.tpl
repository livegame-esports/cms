<div class="col-lg-9 order-is-first">
	<div class="block disp-n" id="notifications_line">
		...
		<div id="notifications">
			<?php if (class_exists("GetData")) { $CE = new GetData($pdo, $tpl); if(method_exists($CE, "notifications")) { $tpl->show($CE->notifications("0", "0", "1")); } unset($CE); } ?>
		</div>
	</div>
...
<script>
	...
	<?php if($conf->show_events != '0'): ?>
	<div class="block">
		<div class="block_head">
			Последние события проекта
		</div> 
		<div id="events">
			<?php if (class_exists("EventsRibbon")) { $CE = new EventsRibbon($pdo, $tpl); if(method_exists($CE, "get_events")) { $tpl->show($CE->get_events(0, 0, $conf->show_events)); } unset($CE); } ?>
		</div>
	</div>
	<?php endif; ?>
</div>
