...
if (isset($_POST['edit_user_vk'])) {
	...
	if(!$U->check_busyness('vk', $user_vk, $_SESSION['id'])) {
		exit('<span class="m-icon icon-remove"></span> Введенный Вами ID занят другим пользователем!');
	}
...
if (isset($_POST['edit_user_steam_id'])) {
	...
	if (!empty($row->id) && $row->id != $_SESSION['id']) {
		exit('<p class="text-danger">Введенный Вами steam id занят пользователем <a href="../profile?id='.$row->id.'" target="_blank">'.$row->login.'</a></p>');
	}
...
if (isset($_POST['edit_user_login'])) {
	...
	if(!$U->check_login_busyness($user_login, $_SESSION['id'])) {
		exit('<span class="m-icon icon-remove"></span> Введенный Вами логин уже зарегистрирован!');
	}
...
if(isset($_POST['editUserRoute'])) {
	...
	if(empty($route)) {
		...
		if(!$U->check_route_busyness($route, $_SESSION['id'])) {
			exit('<span class="m-icon icon-remove"></span> Введенный Вами адрес уже зарегистрирован!');
		}
	}
...
if (isset($_POST['edit_user_nick'])) {
	...
	$row = $STH->fetch();
	if (!empty($row->id) && $row->id != $_SESSION['id']) {
		exit('<p class="text-danger">Введенный Вами ник занят пользователем <a href="../profile?id='.$row->id.'" target="_blank">'.$row->login.'</a></p>');
	}
...
if (isset($_POST['load_friends'])) {
	...
	if(isset($login)) {
		...
		$STH->execute(array(":login" => getNameLike($login)));
	} else {
