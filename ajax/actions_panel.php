...
if(isset($_POST['edit_ip_protect'])) {
	...
	$_SESSION['admin_cache'] = $SC->get_admin_cache(configs()->password);
	exit();
}
if(isset($_POST['edit_site_time_zone'])) {
	...
	$STH->execute([':time_zone' => $time_zone]);
...
if(isset($_POST['edit_copyright_key'])) {
	...
	$STH->execute([':copyright_key' => $copyright_key]);
...
if(isset($_POST['edit_violations_number'])) {
	...
	$STH->execute([':violations_number' => $violations_number]);
...
if(isset($_POST['edit_col_login'])) {
	...
	$STH->execute([':col_login' => $col_login]);
...
if(isset($_POST['edit_violations_delta'])) {
	...
	$STH->execute([':violations_delta' => $violations_delta]);
...
if(isset($_POST['switch_widgets_type'])) {
	...
	$STH = pdo()->prepare(
		"UPDATE config SET widgets_type=:widgets_type, vk_group=:vk_group, vk_admin=:vk_admin LIMIT 1"
	);
	$STH->execute([':widgets_type' => $type, ':vk_group' => '2', ':vk_admin' => '2']);
	...
	if($type == 1) {
		...
		if($row->widgets_type == 1) {
			...
			if($row->vk_api == 2) {
				exit(
				json_encode(
					['status' => '2', 'data' => 'Для работы виджета требуется включить регистрацию через Вконтакте']
				)
				);
			}
			if(empty($row->vk_id) || empty($row->vk_key) || empty($row->vk_service_key)) {
				...
				json_encode(
					[
						'status' => '2',
						'data'   => 'Для работы виджета требуется заполнить все настройки для авторизации через Вконтакте'
					]
				)
				);
			}
			...
			if($row->fb_api == 2) {
				exit(
				json_encode(
					['status' => '2', 'data' => 'Для работы виджета требуется включить регистрацию через Facebook']
				)
				);
			}
			if(empty($row->fb_id) || empty($row->fb_key)) {
				...
				json_encode(
					[
						'status' => '2',
						'data'   => 'Для работы виджета требуется заполнить все настройки для авторизации через Facebook'
					]
				)
				);
			}
...
if(isset($_POST['edit_vk_group_id'])) {
	...
	$STH->execute([':vk_group_id' => $vk_group_id]);
...
if(isset($_POST['edit_vk_admin_id'])) {
	...
	$STH->execute([':vk_admin_id' => $vk_admin_id]);
...
if(isset($_POST['edit_admins_ids'])) {
	...
	$STH->execute([':admins_ids' => $admins_ids]);
...
if(isset($_POST['edit_off_message'])) {
	...
	$STH->execute([':off_message' => $off_message]);
...
if(isset($_POST['edit_show_news'])) {
	...
	$STH->execute([':show_news' => $show_news]);
...
if(isset($_POST['edit_show_events'])) {
	...
	$STH->execute([':show_events' => $show_events]);
...
if(isset($_POST['edit_site_password'])) {
	...
	$old_password = $U->convert_password($old_password, configs()->salt);

	if($old_password != configs()->password) {
	...
	if(isStringLengthMore($password, 17)) {
		exit('<p class="text-danger">Не более 16 символов!</p>');
	}

	$password = $U->convert_password($password, configs()->salt);

	$STH = pdo()->prepare("UPDATE config SET password=:password LIMIT 1");
	$STH->execute([':password' => $password]);

	if(configs()->ip_protect == 1) {
		$SC->admin_ip = get_ip();
	}
...
if(isset($_POST['edit_vk_api'])) {
	...
	$STH = pdo()->prepare(
		"UPDATE config__secondary SET vk_id=:vk_id,vk_key=:vk_key,vk_service_key=:vk_service_key LIMIT 1"
	);
	$STH->execute([':vk_id' => $vk_id, ':vk_key' => $vk_key, ':vk_service_key' => $vk_service_key]);
...
if(isset($_POST['edit_steam_api'])) {
	...
	$STH->execute([':steam_key' => $steam_key]);
	exit('<p class="text-success">Настройки успешно изменены!</p>');
}
if(isset($_POST['edit_fb_api'])) {
	...
	$STH->execute([':fb_id' => $fb_id, ':fb_key' => $fb_key]);
	exit('<p class="text-success">Настройки успешно изменены!</p>');
}
...
if(isset($_POST['edit_interkassa'])) {
	...
	$STH->execute([':ik_login' => $ik_login, ':ik_pass1' => $ik_pass1]);
...
if(isset($_POST['edit_walletone'])) {
	...
	$STH->execute([':wo_login' => $wo_login, ':wo_pass' => $wo_pass]);
...
if(isset($_POST['edit_robokassa'])) {
	...
	$STH->execute([':rb_login' => $rb_login, ':rb_pass1' => $rb_pass1, ':rb_pass2' => $rb_pass2]);
...
if(isset($_POST['edit_webmoney'])) {
	...
	$STH->execute([':wb_login' => $wb_login, ':wb_pass1' => $wb_pass1, ':wb_num' => $wb_num]);
...
if(isset($_POST['editPayAnyWayPaymentSystem'])) {
	$payanyway_id  = check($_POST['payanyway_id'], null);
	$payanyway_code = check($_POST['payanyway_code'], null);

	if(empty($payanyway_id) || empty($payanyway_code)) {
		exit('<p class="text-danger">Вы заполнили не все поля!</p>');
	}

	pdo()
		->prepare("UPDATE config__bank SET payanyway_id=:payanyway_id, payanyway_code=:payanyway_code LIMIT 1")
		->execute([':payanyway_id' => $payanyway_id, ':payanyway_code' => $payanyway_code]);

	write_log("Отредактирован PayAnyWay");
	exit('<p class="text-success">Настройки изменены!</p>');
}

if(isset($_POST['editYooKassaPaymentSystem'])) {
	$yookassa_id  = check($_POST['yookassa_id'], null);
	$yookassa_key = check($_POST['yookassa_key'], null);

	if(empty($yookassa_id) || empty($yookassa_key)) {
		exit('<p class="text-danger">Вы заполнили не все поля!</p>');
	}

	pdo()
		->prepare("UPDATE config__bank SET yookassa_id=:yookassa_id, yookassa_key=:yookassa_key LIMIT 1")
		->execute([':yookassa_id' => $yookassa_id, ':yookassa_key' => $yookassa_key]);

	write_log("Отредактирован YooKassa");
	exit('<p class="text-success">Настройки изменены!</p>');
}
...
if(isset($_POST['dellOldBans'])) {
	$bans = pdo()->query("SELECT id FROM bans WHERE date < (NOW() - INTERVAL 30 DAY) AND closed != '0'");

	while($ban = $bans->fetch(PDO::FETCH_OBJ)) {
		pdo()
			->prepare("DELETE FROM bans__comments WHERE ban_id=:ban_id")
			->execute([':ban_id' => $ban->id]);
	}

	pdo()->exec("DELETE FROM bans WHERE date < (NOW() - INTERVAL 30 DAY) AND closed != '0'");

	removeOldFiles('../files/bans_imgs', 30);
}

if(isset($_POST['dellOldComplaints'])) {
	$complaints = pdo()->query("SELECT id FROM complaints WHERE date < (NOW() - INTERVAL 30 DAY) AND sentence != '0'");

	while($complaint = $complaints->fetch(PDO::FETCH_OBJ)) {
		pdo()
			->prepare("DELETE FROM complaints__comments WHERE complaint_id=:complaint_id")
			->execute([':complaint_id' => $complaint->id]);
	}

	pdo()->exec("DELETE FROM complaints WHERE date < (NOW() - INTERVAL 30 DAY) AND sentence != '0'");

	removeOldFiles('../files/complaints_imgs', 30);
}

if(isset($_POST['dellOldTickets'])) {
	$tickets = pdo()->query("SELECT id FROM tickets WHERE date < (NOW() - INTERVAL 30 DAY) AND closed != '0'");

	while($ticket = $tickets->fetch(PDO::FETCH_OBJ)) {
		pdo()
			->prepare("DELETE FROM tickets__answers WHERE ticket=:ticket")
			->execute([':ticket' => $ticket->id]);
	}

	pdo()->exec("DELETE FROM tickets WHERE date < (NOW() - INTERVAL 30 DAY) AND closed != '0'");

	removeOldFiles('../files/files', 30);
}
...
if(isset($_POST['removeLog'])) {
	...
	(new Logs())->removeLog($slug);

	exit('{}');
}

if(isset($_POST['getLogContent'])) {
	$slug = clean($_POST['slug']);

	exit((new Logs())->getLogContent($slug));
}
...
if(isset($_POST['get_content_tpl']) || isset($_POST['save_code'])) {
	...
	if(
		(!stristr($name, "templates/" . configs()->template . "/tpl/") && !stristr($name, "templates/" . configs()->template . "/css/"))
		|| stristr($name, "..")
		|| stristr($name, "./")
		|| !file_exists($_SERVER["DOCUMENT_ROOT"] . "/" . $name)
		|| (!if_tpl($name) && !if_css($name) && !if_css($name))
	) {
	...
	if(isset($_POST['save_code'])) {
		...
		if(ifSafeMode()) {
			$regExps = array_keys(tpl()->getRegExps());

			$oldExpressions = [];
			$oldContent = file_get_contents($name);

			foreach($regExps as $regExp) {
				$matches = [];
				preg_match_all($regExp, $oldContent, $matches, PREG_PATTERN_ORDER);

				$oldExpressions = array_merge($oldExpressions, $matches[0]);
			}
			...
			foreach($regExps as $regExp) {
				...
				$newExpressions = array_merge($newExpressions, $matches[0]);
			}
			...
			foreach($newExpressions as $newExpression) {
				...
				foreach($oldExpressions as $oldExpression) {
			...
			if(!empty($diff) || (count($newExpressions) != count($oldExpressions))) {
				exit(
					json_encode(
						[
							'status'  => '2',
							'message' => 'Редактирование {} выражений в режиме безопасной эксплуатации запрещено'
						]
					)
				);
			}
		...
		$tpl = new Template;
		$tpl->dell_cache(configs()->template);
		unset($tpl);
...
if(isset($_POST['server_act'])) {
	...
	if($type == '4' || $type == '6') {
		if(empty($db_host) or empty($db_user) or empty($db_pass) or empty($db_db) or empty($db_prefix)) {
			...
			if(!check_column($table, $pdo2, 'nick')) {
				$pdo2->exec("ALTER TABLE $table ADD `nick` VARCHAR(128) DEFAULT ''");
			}
	...
	if($st_type != '0') {
		...
		if($st_type == '1') {
			...
			if(
					(!check_table('hlstats_Players', $pdo2) or !check_table('hlstats_Servers', $pdo2))
					&& !check_table('hlstats_players', $pdo2) or !check_table('hlstats_servers', $pdo2)
			) {
				return_html("Настройки статистики: Структура базы не соответствует данному типу интеграции.", 2, 1);
			}
				...
				<div id="tip5_<?php echo $row->id ?>" class="disp-n">
					<div class="bs-callout bs-callout-info bs-callout-sm mt-5">
						...
						<p>
							...
							<a href="https://gamecms.ru/wiki/game-plugins" target="_blank">GameCMS API(amx)</a> на игровой сервер
						</p>
					</div>
				</div>
				<div id="tip6_<?php echo $row->id ?>" class="disp-n">
					<div class="bs-callout bs-callout-info bs-callout-sm mt-5">
						...
						<p>
							...
							установку плагина <a href="https://gamecms.ru/wiki/game-plugins" target="_blank">GameCMS API(sm)</a> на игровой сервер
						</p>
					</div>
				</div>
...
if(isset($_POST['dell_server'])) {
	...
	while($ban = $STH->fetch()) {
		pdo()->prepare("DELETE FROM bans__comments WHERE ban_id=:id")
			->execute([':id' => $ban->id]);
	}
	...
	while($service = $STH->fetch()) {
		pdo()->prepare("DELETE FROM services__tarifs WHERE service=:id")
	...
	$STH = pdo()->prepare("SELECT id FROM servers__commands WHERE server_id = :server");
	$STH->setFetchMode(PDO::FETCH_OBJ);
	$STH->execute([':server' => $id]);
	while($command = $STH->fetch()) {
		pdo()->prepare("DELETE FROM servers__commands_params WHERE command_id=:id")
			->execute([':id' => $command->id]);
	}
	$STH = pdo()->prepare("DELETE FROM servers__commands WHERE server_id=:server");
	$STH->execute([':server' => $id]);
				...
				<div class="col-md-6">
					...
					<script>
                      $(document).ready(function () {
                        init_tinymce('text<?php echo $idd ?>', '<?php echo md5(configs()->code); ?>', 'lite');
                      });
					</script>
...
if(isset($_POST['send_email_message'])) {
	...
	incNotifications();
	$letter = letter_byadmin(configs()->name, $text);
...
if(isset($_POST['install_module'])) {
	...
	if(empty($_FILES['zip_file']['name'])) {
		...
		$STH->execute([':name' => $module_name, ':tpls' => $tpls, ':info' => $info, ':files' => $files]);
...
if(isset($_POST['install_module_by_key'])) {
	...
	if(is_valid_site("https://minibam.ru")) {
		$r_versions = check_update_version(pdo(), $row->version);
		if(!$r_versions['status']) {
			exit(json_encode(['status' => '2', 'message' => 'Обновите движок, используется устаревшая версия.']));
		} else {
			ignore_user_abort(1);
			set_time_limit(0);
			$result = curl_get_process([
				'website' => "https://minibam.ru/api?type=downloads",
		...
		exit(json_encode(['status' => '2', 'message' => 'Главный сервер не доступен']));
	}
	...
	exit(json_encode(['status' => '1', 'message' => 'Модуль "'.$module_name.'" - успешно установлен']));
}
if(isset($_POST['install_template_by_key'])) {
	...
	if(is_valid_site("https://minibam.ru")) {
		...
		if(!$r_versions['status']) {
			...
			set_time_limit(0);
			$result = curl_get_process([
				'website' => "https://t.minibam.ru/api?type=downloads",
				'data' => "&product=gamecms&resource={$_SERVER['SERVER_NAME']}&code={$_POST['key']}"
			]);
			$result = json_decode(gzdecode($result), true);
			if($result['status'] == 2) {
	...
	exit(json_encode(['status' => '1', 'message' => $template_name]));
}
if(isset($_POST['replace_tpl_img'])) {
	...
	if(
		(
			...
			&& !stristr($folder, "files/merchants/")
			&& !stristr($folder, "files/avatars/")
		)
		|| stristr($folder, "..")
		|| stristr($folder, "./")
	) {
...
if(isset($_POST['get_main_info'])) {
	...
	if(is_valid_site("https://minibam.ru")) {
		...
		if(!$r_versions['status']) {
			...
			if($row->update_link == '') {
			exit(json_encode([
				'version' => "<span class='text-danger'>{$row->version}</span> (Доступно обновление {$remoteVersions[$r_versions['index'] + 1]['version']}: <a class='c-p' onclick='get_update_description();' data-toggle='modal' data-target='#description'>описание</a> | <a class='c-p' onclick='get_update();'>получить</a>)",
				'message' => $remoteVersions[$r_versions['index'] + 1]['description']
			]));
			} else {
			exit(json_encode([
				'version' => "<span class='text-danger'>{$row->version}</span> (Доступно обновление {$remoteVersions[$r_versions['index'] + 1]['version']}: <a class='c-p' onclick='get_update_description();' data-toggle='modal' data-target='#description'>описание</a> | <a class='c-p' onclick='install_update();'>установить</a>)",
				'message' => $remoteVersions[$r_versions['index'] + 1]['description']
			]));
			}
		}
...
if(isset($_POST['get_update_description'])) {
	...
	if(is_valid_site("https://minibam.ru")) {
		...
		if(!$r_versions['status']) {
			$remoteVersions = $r_versions['versions'];
			exit(
				curl_get_process(['website' => "https://minibam.ru/gcms/upinfo/{$remoteVersions[$r_versions['index'] + 1]['version']}"])
			);
		} else {
			exit(json_encode([
				'version' => "{$row->version} (Используется актуальная версия)"
...
if(isset($_POST['get_update'])) {
	...
	$dirExists = "{$_SERVER['DOCUMENT_ROOT']}/modules/updates";
	if(file_exists($dirExists)) {
		if(removeDirectory($dirExists))
			mkdir("{$_SERVER['DOCUMENT_ROOT']}/modules/updates", 0777);
	}
	else mkdir("{$_SERVER['DOCUMENT_ROOT']}/modules/updates", 0777);
	$STH = pdo()->query("SELECT version, update_link FROM config__secondary LIMIT 1");
	$STH->setFetchMode(PDO::FETCH_OBJ);
	$row = $STH->fetch();
...
if(isset($_POST['dell_cache'])) {
	pdo()->prepare("UPDATE config SET cache=:cache LIMIT 1")
...
if(isset($_POST['save_forum_settings'])) {
	...
	foreach($extGroups as $key => $extGroup) {
		foreach(getForbiddenExtensionEntry() as $forbiddenExtEntry) {
			if(stristr($extGroup, $forbiddenExtEntry) !== false) {
				exit (json_encode(['status' => '2', 'input' => $key, 'data' => 'Недопустимые значения']));
			}
		}
	}
