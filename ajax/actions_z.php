...
if (!isPostRequest() || !isRightToken() || (!is_auth() && !is_admin())) {
...
if (isset($_POST['give_money']) and is_worthy("m")) {
	...
	$STH = $pdo->query("SELECT id,shilings FROM users WHERE id='$id' LIMIT 1");
...
if (isset($_POST['take_proc']) and is_worthy("c")) {
	...
	$STH = $pdo->query("SELECT id FROM users WHERE id='$id' LIMIT 1");
	$STH->setFetchMode(PDO::FETCH_OBJ);
	$row = $STH->fetch();
	if (empty($row->id)) {
		exit(json_encode(array('status' => '2')));
...
if (isset($_POST['dell_user']) and (is_worthy("g") or is_admin())) {
	...
	$STH = $pdo->query("SELECT id,login,avatar FROM users WHERE id='$id' LIMIT 1");
	$STH->setFetchMode(PDO::FETCH_OBJ);
	$row = $STH->fetch();
	if (empty($row->id)) {
		exit(json_encode(array('status' => '2')));
	...
	if ($type == 1 || $type == 4 || $type == 2) {
		$Forum = new Forum($pdo);
		$STH = $pdo->prepare("SELECT `id` FROM `forums__topics` WHERE `author`=:author");
		$STH->setFetchMode(PDO::FETCH_OBJ);
		$STH->execute(array(':author' => $id));
		while ($row2 = $STH->fetch()) {
			$Forum->dell_topic($row2->id);
		}
		$STH = $pdo->prepare("SELECT `id` FROM `forums__messages` WHERE `author`=:author");
		$STH->setFetchMode(PDO::FETCH_OBJ);
		$STH->execute(array(':author' => $id));
		while ($row2 = $STH->fetch()) {
	...
	if ($type == 1 || $type == 2) {
		...
		$STH = $pdo->query("SELECT id, img FROM bans WHERE author='$id'");
		$STH->execute();
		$row = $STH->fetchAll();
		for ($i = 0; $i < count($row); $i++) {
			$ban_id = $row[$i]['id'];
			...
			if (isset($row[$i]['img']) and $row[$i]['img'] != '0') {
				...
				for ($j = 0; $j < count($data); $j++) {
					if (!empty($data[$j]) && file_exists('../' . $data[$j])) {
						unlink('../' . $data[$j]);
					}
				}
			}
		...
		$STH = $pdo->query("SELECT id, screens FROM complaints WHERE author_id='$id' OR accused_profile_id='$id'");
		$STH->execute();
		$row = $STH->fetchAll();
		for ($i = 0; $i < count($row); $i++) {
		...
		$STH = $pdo->query("SELECT id, files FROM tickets WHERE author='$id'");
		$STH->execute();
		$row = $STH->fetchAll();
		for ($i = 0; $i < count($row); $i++) {
			$ticket = $row[$i]['id'];

			if (isset($row[$i]['files']) && $row[$i]['files'] != 'none' && file_exists('../' . $row[$i]['files'])) {
				unlink('../' . $row[$i]['files']);
			}
...
if (isset($_POST['dell_multi_account_relation']) and (is_worthy("f") or is_admin())) {
	...
	$STH = $pdo->prepare("SELECT `id`, `multi_account` FROM `users` WHERE `id`=:id OR `id`=:id_second LIMIT 2");
	$STH->setFetchMode(PDO::FETCH_OBJ);
...
if (isset($_POST['admin_change_avatar']) and (is_worthy("f") or is_admin())) {
	...
	if (empty($_FILES['user_avatar']['name'])) {
		...
		if (if_img($_FILES['user_avatar']['name'])) {
			...
			$date = time();
			$imageName = $filename . $date;
			clip_image($im, 300, $path . $imageName);
			$user_avatar = $path . $imageName . ".jpg";
			unlink('../' . $path . $filename);

			$STH = $pdo->query("SELECT avatar FROM users WHERE id='$id'");
			$STH->setFetchMode(PDO::FETCH_OBJ);
			$tmp = $STH->fetch();
...
if (isset($_POST['admin_change_login']) and (is_worthy("f") or is_admin())) {
	...
	if (!$U->check_login_busyness($login)) {
		exit(json_encode(array('status' => '2', 'data' => '<p class="text-danger">Введенный логин уже зарегистрирован!</p>')));
	}

	$STH = $pdo->prepare("UPDATE users SET login=:login WHERE id='$id' LIMIT 1");
	$STH->execute(array(':login' => $login));

	$STH = $pdo->prepare("SELECT `email` FROM `users` WHERE `id`=:id LIMIT 1");
	$STH->setFetchMode(PDO::FETCH_OBJ);
...
if (isset($_POST['editUserRouteByAdmin']) and (is_worthy("f") or is_admin())) {
	...
	if (empty($route)) {
		...
		if (!$U->check_route_busyness($route)) {
			exit(json_encode(['status' => 2, 'data' => '<p class="text-danger">Введенный Вами адрес уже зарегистрирован!</p>']));
		}
	}
...
if (isset($_POST['admin_change_nick']) and (is_worthy("f") or is_admin())) {
	...
	$STH = $pdo->query("SELECT id,login FROM users WHERE nick='$user_nick' LIMIT 1");
	$STH->setFetchMode(PDO::FETCH_OBJ);
	$row = $STH->fetch();
	if (!empty($row->id)) {
		exit(json_encode(array('status' => '2', 'data' => 'Введенный Вами ник занят пользователем ' . $row->login)));
	}
...
if (isset($_POST['load_servers_admins']) and (is_worthy("j") or is_admin())) {
	...
	$STH = $pdo->prepare("SELECT `id`,`name`,`type` FROM `servers` WHERE `id`=:id LIMIT 1");
	$STH->setFetchMode(PDO::FETCH_OBJ);
	...
	if ($server->type == 0) {
		...
		while ($row = $STH->fetch()) {
			$i++;
			$STH2 = $pdo->prepare("SELECT `services`.`name`, `services`.`show_adm`, `admins__services`.`ending_date` FROM `admins__services` LEFT JOIN `services` ON `admins__services`.`service` = `services`.`id` WHERE `admins__services`.`admin_id`=:admin_id LIMIT 10");
...
if (isset($_POST['load_edit_admin_result']) and (is_worthy("j") or is_admin())) {
	...
	$STH = $pdo->prepare("SELECT `services`.`name` FROM `admins__services` LEFT JOIN `services` ON `admins__services`.`service` = `services`.`id` WHERE `admins__services`.`admin_id`=:admin_id LIMIT 10");
	$STH->setFetchMode(PDO::FETCH_OBJ);
...
if (isset($_POST['resume_admin']) and (is_worthy("j") or is_admin())) {
	...
	$STH = $pdo->prepare("SELECT `ending_date`, `id` FROM `admins__services` WHERE `admin_id`=:admin_id");
	$STH->setFetchMode(PDO::FETCH_OBJ);
	$STH->execute(array(':admin_id' => $id));
	while ($row = $STH->fetch()) {
...
if (isset($_POST['edit_admin']) and (is_worthy("j") or is_admin())) {
	...
	$old_name = null;
	if ($type == 'user_id') {
		$STH = $pdo->prepare("SELECT `id`, `nick` FROM `users` WHERE `id`=:id LIMIT 1");
		$STH->setFetchMode(PDO::FETCH_OBJ);
...
if (isset($_POST['add_admin']) and (is_worthy("j") or is_admin())) {
	...
	$STH = $pdo->query("SELECT id,type,ip,port,name,pass_prifix,discount,binds FROM servers WHERE id='$server' LIMIT 1");
	$STH->setFetchMode(PDO::FETCH_OBJ);
	$server = $STH->fetch();
	if (empty($server->id)) {
	...
	$STH = $pdo->query("SELECT id,price,time,discount FROM services__tarifs WHERE id='$tarif' and service='$service->id' LIMIT 1");
	$STH->setFetchMode(PDO::FETCH_OBJ);
	$tarif = $STH->fetch();
	if (empty($tarif->id)) {
	...
	$STH = $pdo->query("SELECT id,login,nick,email,proc,rights FROM users WHERE id='$admin[user_id]' LIMIT 1");
	$STH->setFetchMode(PDO::FETCH_OBJ);
	$user = $STH->fetch();
	if (empty($user->id)) {
	...
	if ($server->type == 4) {
		...
		if ($admin['has_rights'] == 1) {
			if ($service->sb_group != '') {
				$STH = $pdo->query("SELECT `admins__services`.`id` FROM `admins__services` LEFT JOIN `services` ON `admins__services`.`service` = `services`.`id` WHERE `services`.`sb_group`!='' AND `admins__services`.`admin_id` = '$admin[id]' LIMIT 1");
				$STH->setFetchMode(PDO::FETCH_OBJ);
				$row = $STH->fetch();
				if (isset($row->id)) {
...
if (isset($_POST['get_tarifs']) and (is_worthy("j") or is_admin())) {
	...
	tpl()->result['options'] = '';
	$STH = $pdo->query("SELECT id,time FROM services__tarifs WHERE service = '$id' ORDER BY time");
	$STH->setFetchMode(PDO::FETCH_OBJ);
	while ($row = $STH->fetch()) {
		if ($row->time == 0) {
			$time = 'Навсегда';
...
if (isset($_POST['get_user_shilings_operations']) and (is_worthy("f") or is_admin())) {
	...
	$STH = $pdo->query("SELECT `money__actions`.*, `money__actions_types`.`name`, `money__actions_types`.`class` FROM `money__actions` 
		...
		WHERE `money__actions`.`author`='$id' ORDER BY `money__actions`.`date` DESC");
	$STH->setFetchMode(PDO::FETCH_OBJ);
	while ($row = $STH->fetch()) {
		$i++;
