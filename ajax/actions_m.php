	...
	try {
		...
		switch($cashierSlug) {
			...
			case 'payanyway':
				...
				$monetaSDK->setSettingValue('monetasdk_success_url',  $full_site_host . 'purse?result=success');
				$monetaSDK->setSettingValue('monetasdk_fail_url',  $full_site_host . 'purse?result=fail');
			...
			case 'yookassa':
				...
				$payment = $client->createPayment(
					[
						'amount'       => [
							'value'    => $amount,
							'currency' => Payments::getCashierCurrency(
								'yookassa'
							),
						],
						'confirmation' => [
...
if(isset($_POST['get_operations'])) {
	...
	$tpl      = new Template;
	$tpl->dir = '../templates/' . configs()->template . '/tpl/';
