...
if (isset($_POST['user_visit'])) {
	...
	$STH = $pdo->query("SELECT * FROM users__visit WHERE user_id='$id' and user_visit='$_SESSION[id]' ORDER BY id DESC LIMIT 1"); $STH->setFetchMode(PDO::FETCH_OBJ);  
	$row = $STH->fetch();
	$visitTime = strtotime($row->date);
	$lastTime = $visitTime+86401;
	$nowTime = time();
...
if (isset($_POST['get_user_visit'])) {
	...
	$STH = $pdo->query("SELECT users__visit.*, users.login, users.avatar, users.rights FROM users__visit LEFT JOIN users ON users__visit.user_visit = users.id WHERE user_id='$id' and date='$date'"); $STH->setFetchMode(PDO::FETCH_OBJ); 
	...
	if ($i == 0){
		echo '<span class="empty-element">Сегодня никто не посещал!</span>';
	} else {
